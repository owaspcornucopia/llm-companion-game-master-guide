...
The game takes the participants through a provocative scenario where they have to identify AI threats by studying an insecure AI implementation. They need to ask themselves "what can go wrong" and "what we are going to do about it"? Furthermore, by playing, they will get to know which tests from the OWASP AI Test Guide (AITG) and OWASP AI Security Verification Standard (AISVS) need to be considered in order to responsibly develop AI applications.
...
See the notes in the presentation for instructions for how to run this table top session.
When playing with multiple teams let one person on the team lead the session. This person will be the team's game master and will recieve the link to the cheat sheet. Show the person the [QR code](/images/cheatsheets-qr-code-link.png) for the cheat sheets.
...
- (Not included yet) Ethical AI Principles: Prioritizing human oversight, safety, and non-discrimination during the design and testing phases.
